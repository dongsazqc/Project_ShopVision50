...
export default function Promotions() {
  ...
  const parseCondition = (condition) => {
    ...
    parts.forEach((p) => {
      if (p.startsWith("min:")) result.minOrderAmount = parseInt(p.replace("min:", ""), 10);
  ...
  const handleSave = async (values) => {
    try {
      ...
      const condition = `min:${values.minOrderAmount || 0}${
        values.items?.length ? `;items:${JSON.stringify(values.items)}` : ""
      }`;
        ...
        <Form layout="vertical" form={form} onFinish={handleSave}>
          <Form.Item
            ...
            rules={[
              ...
              { min: 5, max: 20, message: "Mã khuyến mãi phải có từ 5-20 ký tự" },
              { pattern: /^[A-Za-z0-9]+$/, message: "Chỉ chấp nhận chữ và số" },
            ]}
          >
          ...
          <Form.Item
            ...
            name="scope"
            rules={[{ required: true, message: "Chọn phạm vi áp dụng" }]}
          >
            <Select
              options={[
                { label: "Tất cả sản phẩm", value: "all" },
                { label: "Nhóm sản phẩm", value: "category" },
                { label: "Sản phẩm riêng lẻ", value: "product" },
              ]}
            />
          </Form.Item>

          {["category", "product"].includes(form.getFieldValue("scope")) && (
            <Form.Item
              label={
