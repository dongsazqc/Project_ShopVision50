...
export default function Products() {
    ...
    const fetchProducts = async () => {
        try {
            ...
            list = list.map((p) => ({
                ...p,
                sanPhamId: p.productId, // FE dùng sanPhamId → map lại
            }));

            // SORT MỚI NHẤT LÊN ĐẦU
            list = list.sort(
                (a, b) => Number(b.sanPhamId) - Number(a.sanPhamId)
            );
    ...
    const fetchMetaData = async () => {
        try {
            ...
            console.error(err);
            messageApi.error("Không thể tải dữ liệu meta");
        }
    };
    ...
    const fetchColorAndSizes = async () => {
        try {
            ...
            messageApi.error("Không thể tải màu hoặc size");
        }
    };
    ...
    const fetchVariants = async (productId) => {
        ...
        try {
            ...
            console.error(err);
            messageApi.error("Không thể tải biến thể");
        }
    };
    ...
    const submitVariant = async (values) => {
        try {
            if (!editingProduct?.productId) {
                messageApi.warning("Sản phẩm chưa được lưu.");
                return;
            }
            ...
            if (duplicate) {
                messageApi.error("Biến thể với màu và kích cỡ này đã tồn tại");
                return;
            }

            const giaGoc = editingProduct?.price || editingProduct?.giaGoc || 0;
            const discount = values.discountPercent ?? 0;

            // giá bán = giá gốc - (giá gốc * % giảm)
            const calculatedGiaBan = giaGoc - (giaGoc * discount) / 100;
    ...
    //   } catch (err) {
    //     console.error(err);
    //     messageApi.error("Không thể đặt ảnh chính");
    //   }
    // };
    ...
    const columns = [
        ...
        //   width: 100,
        //   render: (_, record) => {
        //     // productImages có thể = null → chuyển thành []
...

        {
            ...
            dataIndex: "originId",
            render: (val) => {
                const found = metaData.xuatxus?.find((x) => x.originId === val);
        ...
        <div>
            ...
            >
                <Form
                    ...
                    onValuesChange={(changedValues) => {
                        // Khi đổi % khuyến mãi → tự tính giá bán
                        if (changedValues.discountPercent !== undefined) {
                            const giaGoc =
                                editingProduct?.giaGoc ||
                                editingProduct?.price ||
                                0;
                            const discount = changedValues.discountPercent ?? 0;
                            const newPrice = giaGoc - (giaGoc * discount) / 100;
