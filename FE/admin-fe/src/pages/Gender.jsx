...
export default function Gender() {
    ...
    const fetchGenders = async () => {
        try {
            ...
            console.error(err);
            messageApi.error("Không thể tải danh sách giới tính");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAdd = async (values) => {
        try {
            ...
            if (exists) {
                messageApi.warning("Tên giới tính đã tồn tại!");
                return;
            }

            await api.post("/Gender/CreateGender", { name });
            messageApi.success("Thêm giới tính thành công!");
            setAddModalOpen(false);
            formAdd.resetFields();
            fetchGenders();
        } catch (err) {
            console.error(err);
            messageApi.error("Không thể thêm giới tính");
        } finally {
            setSaving(false);
        }
    };
    ...
    const openEditModal = async (record) => {
        try {
            const res = await api.get(
                `/Gender/GetGenderById/${record.genderId}`
            );
            ...
            console.error(err);
            messageApi.error("Không thể tải thông tin giới tính");
        }
    };
    ...
    const handleUpdate = async (values) => {
        try {
            ...
            messageApi.success("Cập nhật giới tính thành công!");
            setEditModalOpen(false);
            setSelectedGender(null);
            fetchGenders();
        } catch (err) {
            console.error(err);
            messageApi.error("Không thể cập nhật giới tính");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            messageApi.success("Xóa giới tính thành công!");
            fetchGenders();
        } catch (err) {
            console.error(err);
            messageApi.error("Không thể xóa giới tính");
        }
    };
    ...
    const columns = [
        {
            ...
            key: "genderId",
            width: 80,
            sorter: (a, b) => a.genderId - b.genderId,
    ...
    return (
        <div>
            ...
            <Table
                dataSource={[...genders].sort(
                    (a, b) => b.genderId - a.genderId
                )}
                columns={columns}
                loading={loading}
                rowKey="genderId"
                                ...
                                validator: (_, value) => {
                                    if (!value || !value.trim())
                                        return Promise.reject(
                                            "Tên giới tính không được chỉ chứa khoảng trắng"
                                        );
