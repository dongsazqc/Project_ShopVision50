...
export default function StylePage() {
    ...
    const fetchStyles = async () => {
        try {
            ...
            console.error(err);
            messageApi.error("Không thể tải danh sách phong cách");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAdd = async (values) => {
        try {
            ...
            if (exists) {
                messageApi.warning("Tên phong cách đã tồn tại!");
                return;
            }

            await api.post("/Style", { name });
            messageApi.success("Thêm phong cách thành công!");
            setAddModalOpen(false);
            formAdd.resetFields();
            fetchStyles();
        } catch (err) {
            console.error(err);
            messageApi.error("Không thể thêm phong cách");
        } finally {
            setSaving(false);
        }
    };
    ...
    const openEditModal = async (record) => {
        try {
            ...
            console.error(err);
            messageApi.error("Không thể tải thông tin phong cách");
        }
    };
    ...
    const handleUpdate = async (values) => {
        try {
            ...
            messageApi.success("Cập nhật phong cách thành công!");
            setEditModalOpen(false);
            setSelectedStyle(null);
            fetchStyles();
        } catch (err) {
            console.error(err);
            messageApi.error("Không thể cập nhật phong cách");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            messageApi.success("Xóa phong cách thành công!");
            fetchStyles();
        } catch (err) {
            console.error(err);
            messageApi.error("Không thể xóa phong cách");
        }
    };
                                ...
                                validator: (_, value) => {
                                    if (!value || !value.trim()) {
                                        return Promise.reject(
                                            "Tên phong cách không được chỉ chứa khoảng trắng"
                                        );
                                    }
