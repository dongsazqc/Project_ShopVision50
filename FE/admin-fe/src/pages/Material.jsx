...
export default function Material() {
    ...
    const fetchMaterials = async () => {
        try {
            ...
            console.error(err);
            messageApi.error("Không thể tải danh sách chất liệu");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAdd = async (values) => {
        try {
            ...
            if (exists) {
                messageApi.warning("Tên chất liệu đã tồn tại!");
                return;
            }
            ...
            console.error(err);
            messageApi.error("Không thể thêm chất liệu");
        } finally {
            setSaving(false);
        }
    };
    ...
    const openEditModal = async (record) => {
        try {
            const res = await api.get(
                `/Material/GetMaterialById/${record.materialId}`
            );
            ...
            console.error(err);
            messageApi.error("Không thể tải thông tin chất liệu");
        }
    };
    ...
    const handleUpdate = async (values) => {
        try {
            ...
            await api.put(
                `/Material/UpdateMaterial/${selectedMaterial.materialId}`,
                payload
            );
            ...
            console.error(err);
            messageApi.error("Không thể cập nhật chất liệu");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            ...
            console.error(err);
            messageApi.error("Không thể xóa chất liệu");
        }
    };
    ...
    return (
        <div>
            ...
            <Table
                dataSource={[...materials].sort(
                    (a, b) => b.materialId - a.materialId
                )}
                columns={columns}
                loading={loading}
                rowKey="materialId"
                                ...
                                validator: (_, value) => {
                                    if (!value || !value.trim()) {
                                        return Promise.reject(
                                            "Tên chất liệu không được chỉ chứa khoảng trắng"
                                        );
                                    }
