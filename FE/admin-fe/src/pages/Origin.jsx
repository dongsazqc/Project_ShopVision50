...
export default function Origin() {
  ...
  const fetchOrigins = async () => {
    try {
      ...
      console.error(err);
      message.error("Không thể tải danh sách xuất xứ");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearch = (value) => {
    if (!value.trim()) {
      ...
      setOrigins(
        allOrigins.filter((o) =>
          (o.country || "").toLowerCase().includes(value.toLowerCase())
        )
      );
    }
  };
  ...
  const handleAdd = async (values) => {
    try {
      ...
      const exists = allOrigins.some(
        (o) => (o.country || "").trim().toLowerCase() === country.toLowerCase()
      );
      if (exists) {
        message.warning("Tên nước đã tồn tại!");
        return;
      }
      ...
      console.error(err);
      message.error("Không thể thêm xuất xứ");
    } finally {
      setSaving(false);
    }
  };
  ...
  const openEditModal = async (record) => {
    try {
      ...
      console.error(err);
      message.error("Không thể tải thông tin xuất xứ");
    }
  };
  ...
  const handleUpdate = async (values) => {
    try {
      ...
      const exists = allOrigins.some(
        (o) =>
          (o.country || "").trim().toLowerCase() === country.toLowerCase() &&
      ...
      console.error(err);
      message.error("Không thể cập nhật xuất xứ");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error(err);
      message.error("Không thể xóa xuất xứ");
    }
  };

  // ================= CỘT HIỂN THỊ =================
  const columns = [
    { title: "ID", dataIndex: "originId", key: "originId", width: 80, sorter: (a, b) => a.originId - b.originId, defaultSortOrder: "descend" },
    {
      title: "Tên nước",
      dataIndex: "country",
      key: "country",
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
        <Input.Search placeholder="Tìm theo tên nước..." onSearch={handleSearch} allowClear style={{ width: 300 }} />
      ...
      <Table
        dataSource={[...origins].sort((a, b) => b.originId - a.originId)}
        columns={columns}
        loading={loading}
        rowKey="originId"
        bordered
        ...
        <Form layout="vertical" form={formAdd} onFinish={handleAdd} preserve={false}>
          <Form.Item
            ...
            rules={[
              ...
              {
                validator: (_, value) => {
                  if (!value || !value.trim()) return Promise.reject("Tên nước không được chỉ chứa khoảng trắng");
                  const name = value.trim().toLowerCase();
                  if (allOrigins.some(o => (o.country || "").trim().toLowerCase() === name)) {
                    return Promise.reject("Tên nước đã tồn tại");
                  }
            ...
            rules={[
              { required: true, message: "Nhập tên nước" },
              {
                validator: (_, value) => {
                  if (!value || !value.trim()) return Promise.reject("Tên nước không được chỉ chứa khoảng trắng");
                  const name = value.trim().toLowerCase();
                  if (allOrigins.some(o => (o.country || "").trim().toLowerCase() === name && o.originId !== selectedOrigin?.originId)) {
                    return Promise.reject("Tên nước đã tồn tại");
                  }
                  return Promise.resolve();
                },
